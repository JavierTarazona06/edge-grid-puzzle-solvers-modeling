...
\input{generated/palisade_analysis_macros}
...
\input{generated/palisade_table_families}
...
\input{generated/palisade_table_performance}
...
\input{generated/palisade_table_contrasts}

Pour les instances contrastées de type \(8 \times 5\), un seul temps heuristique exploitable est disponible (\SI{0.28}{\second}). Nous ne l'affichons pas dans la figure suivante, car un point isolé ne permet pas de comparer utilement le comportement de l'heuristique dans ce scénario.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.78\textwidth]{imgs/palisade/palisade_contrast_indices_8x5_3d.pdf}
    \caption{Relation entre la composition des indices et le temps de résolution pour les instances contrastées de type \(8 \times 5\), pour CPLEX uniquement. L'axe \(x\) correspond au nombre d'indices de valeur 2, l'axe \(y\) au nombre total d'indices de valeur 1 et 3, et l'axe \(z\) au temps de résolution.}
    \label{fig:palisade-contrast-indices-8x5-3d}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.78\textwidth]{imgs/palisade/palisade_contrast_indices_10x5_3d.pdf}
    \caption{Relation entre la composition des indices et le temps de résolution pour les instances contrastées de type \(10 \times 5\). L'axe \(x\) correspond au nombre d'indices de valeur 2, l'axe \(y\) au nombre total d'indices de valeur 1 et 3, et l'axe \(z\) au temps de résolution.}
    \label{fig:palisade-contrast-indices-10x5-3d}
\end{figure}
